...
fn test_encrypt_decrypt_team_dek() {
    ...
    // Decrypt with same member DEK
    let decrypted = manager.decrypt_team_dek_for_member(&encrypted, &member_dek);

    // Verify decryption succeeded
    assert!(decrypted.is_some(), "Decryption should succeed with correct key");
...
fn test_encrypt_decrypt_fails_with_wrong_key() {
    ...
    // Try to decrypt with member2 DEK (wrong key)
    let decrypted = manager.decrypt_team_dek_for_member(&encrypted, &member2_dek);
...
fn test_encrypt_decrypt_different_accounts() {
    ...
    // But both should decrypt to same team DEK
    let decrypted1 = manager.decrypt_team_dek_for_member(&encrypted1, &member_dek);
    let decrypted2 = manager.decrypt_team_dek_for_member(&encrypted2, &member_dek);
...
fn test_full_team_key_lifecycle() {
    ...
    // 6. Verify cross-decryption fails (member1 key cannot decrypt member2's encrypted DEK)
